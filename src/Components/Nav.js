...
export default function Nav(props) {
  const styles =
    props.font === "brown" ? { color: "#8B4513" } : { color: "white" };

  const logo = props.font === "brown" ? modeva2 : modeva;
  return (
    <div
      className=" Nav flex w-full items-center justify-evenly mt-10 max-h-20"
      style={{ gridArea: "nav" }}
    >
      <div className="logo">
        <a>
      ...
      <div
        className="!hidden xl:!flex text-xl nav items-center justify-evenly gap-4 m-auto"
        style={styles}
      >
        <a>Catalog</a>
        <a>Sale</a>
      ...
      <div className="flex items-center justify-end max-h-12">
        <form className="flex items-center justify-end max-w-[40%]">
          <span
            className="material-symbols-outlined h-1/3 text-gray-500"
